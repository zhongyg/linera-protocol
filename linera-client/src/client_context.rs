    ...
    W: Persist<Target = Wallet>,
{
    ...
    pub async fn make_benchmark_chains(
        ...
        let key_pair_and_chain_client = self
            .wallet
            .owned_chain_ids()
            .into_iter()
            .map(|chain_id| {
                ...
                let key_pair = self
                    ...
                    .unwrap();
                Ok::<(KeyPair, ChainClient<NodeProvider, S>), Error>((
                    key_pair,
                    self.make_chain_client(chain_id)?,
                ))
            })
        ...
        for (key_pair, chain_client) in key_pair_and_chain_client {
        ...
        info!("Updating wallet from clients");
        for child_client in chain_clients {
